...
#include "x86_64_accton_as7327_56x_int.h"
...
const int sfp_map[] =  {
        ...
        149,150,151,152,153,154,155,156};    /*port 49~56 QSFP*/
...
onlp_sfpi_presence_bitmap_get(onlp_sfp_bitmap_t* dst)
{
    ...
    for (addr = 62; addr <= 64; addr+=2) {
        ...
        if (length == 2) {
            ...
            count = fscanf(fp, "%x %x %x %x %x %x", ptr+0, ptr+1, ptr+2, ptr+3, ptr+4, ptr+5);
        }
        fclose(fp);
    ...
    presence_all <<= 4;
    presence_all |= bytes[1];

    presence_all <<= 8;
    presence_all |= bytes[0];
...
onlp_sfpi_rx_los_bitmap_get(onlp_sfp_bitmap_t* dst)
{
    ...
    uint32_t *ptr = bytes;
    FILE* fp;
    unsigned long long rx_los_all = 0 ;
